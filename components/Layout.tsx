...
import { LayoutDashboard, Upload, Receipt, Settings, PieChart, Package, Store, LogOut } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ currentView, onChangeView, children }) => {
  const navItems = [
    ...
    { view: AppView.LOGOUT, label: 'Logout', icon: LogOut },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 text-slate-800 font-sans">
      ...
      <aside className="fixed bottom-0 z-50 w-full bg-white border-t border-gray-200 md:relative md:w-64 md:border-t-0 md:border-r md:flex md:flex-col md:h-screen">
        ...
        <nav className="flex justify-around md:flex-col md:justify-start md:p-4 md:space-y-2 h-16 md:h-full overflow-x-auto md:overflow-visible">
          {navItems.map((item) => (
            <button
              key={item.view}
              onClick={() => onChangeView(item.view)}
              className={`flex flex-col md:flex-row items-center flex-shrink-0 md:px-4 md:py-3 rounded-lg transition-colors md:w-full p-2 md:p-3 ${item.view === AppView.LOGOUT ? 'md:mt-auto text-red-500 hover:bg-red-50 hover:text-red-700' :
